...
function checkers_read_json_input(): array {
...
function checkers_json_error(string $message, int $code = 400): void {
    http_response_code($code);
    echo json_encode(['success' => false, 'error' => $message], JSON_UNESCAPED_UNICODE);
    exit();
}

function checkers_resolve_lang(array $input): string {
    $lang = abs_resolve_lang($input);
    return $lang === 'en' ? 'en' : 'ru';
}

function checkers_default_nickname($db, string $lang): string {
    $uid = user_current_id();
    if ($uid !== null) {
        $profile = user_login_row($db, $uid);
        if (is_array($profile) && !empty($profile['username'])) {
            return checkers_normalize_player_name((string) $profile['username']);
        }
    }
