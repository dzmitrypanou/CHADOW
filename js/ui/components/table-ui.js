const TableUI = {
    ...
    updateTeamTabs(showEnemy) {
        ...
        if (existingTabs) {
            const buttons = existingTabs.querySelectorAll('.team-tab');
            if (buttons.length === 2) {
                buttons[0].textContent = isEn ? 'Allied team' : 'Команда союзников';
                buttons[1].textContent = isEn ? 'Enemy team' : 'Команда противников';
                buttons[0].classList.toggle('active', !showEnemy);
                buttons[1].classList.toggle('active', showEnemy);
                return;
            }
            existingTabs.remove();
        }
        ...
        if (!anchor) return;
        const mapTags = container.querySelector('.map-tags');
        if (mapTags) {
            mapTags.insertAdjacentElement('afterend', tabs);
        } else {
    ...
    updateMapTags() {
        ...
        const anchor = this.getPlayersTableHeaderAnchor(container);
        if (!anchor) return;
        const teamTabs = container.querySelector('.team-tabs');
        if (teamTabs) {
            teamTabs.insertAdjacentElement('beforebegin', tagsContainer);
        } else {
