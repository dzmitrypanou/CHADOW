...
set "MOD_NAME=chadow.battle-limit"
set "MOD_ENTRY=mod_chadow_battle_limit.py"
...
if errorlevel 1 (
  echo [ERROR] Python 2.7 compile failed. Check python --version
  exit /b 1
)
...
move /Y "%folder%" "..\%folder%" >nul
popd

echo Built ..\%MOD_NAME%_%v%.mtmod
endlocal
